...
直近の状態: Path A単一運用を文書/実装で再確認。Playwright統合テストはCodex sandbox環境ではskipガードを導入。docsの旧コマンド参照(run_csv_pipeline.py)を現行導線(Web UI CSV Pipeline)へ整理し、CIにドキュメント参照Lintを追加。Python非integration 69 passed, 1 deselected / .NET build success。
