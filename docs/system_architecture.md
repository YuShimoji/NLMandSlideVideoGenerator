...
flowchart LR
    ...
    subgraph S2[Stage 2: ビジュアル+CSV生成]
        ...
        B2[StockImageClient<br/>Pexels/Pixabay/Wikimedia]
...
- **VisualResourceOrchestrator** (`src/core/visual/resource_orchestrator.py`): 全ビジュアルリソースを統合。Pexels→Pixabay→Wikimedia Commons→placeholderのフォールバック連鎖、連続source多様性強制、アニメーション自動割当。
...
sequenceDiagram
    ...
    Stock-->>Orch: StockImage[] (Pexels/Pixabay/Wikimedia)
...
1. **ビジュアルリソース**: Pexels → Pixabay → Wikimedia Commons → placeholder
...
- visual: StockImageClient (Pexels→Pixabay→Wikimedia Commons) → placeholder
- textual: placeholder (STATIC アニメーション)
...
# テスト
pytest                   # 1346テストPASS (2026-03-22時点)
```
...
│   ├── thumbnails/
│   │   └── ymm4_thumbnail_generator.py  # YMM4テンプレートサムネイル生成 (PILフォールバック)
│   ├── timeline/
