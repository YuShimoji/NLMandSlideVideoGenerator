...
- 対象とする範囲:
  ...
  - OS: Windows + YMM4 + .NET 10 SDK が導入済みであるローカル環境（YMM4 プラグインサンプルに追随）。
...
- YMM4 プラグイン API のインターフェース仕様（特に `IVoicePlugin` / `ITextCompletionPlugin` のメソッド定義）は、2025-12-11 時点の公開ドキュメントでは詳細が確認できず、Y4 での本格実装は backlog C3-1/C3-2 の保留解消後を想定。
...
- 前提:
  - .NET 10 SDKインストール済み（YMM4 プラグインサンプルに追随）
...
- 想定アウトプット:
  ...
  - ビルド・デプロイ手順書（`ymm4-plugin/README.md` に集約）
