...
**最終更新**: 2026-03-17
**ステータス**: Phase 0-4 全完了。E2E全PASS (2026-03-16)、BGMテンプレート完了 (2026-03-17)
...
- テンプレート: `config/style_template.json` v1.1 で統一管理 **実装済み**
  - `video`、`subtitle`、`animation`、`bgm`、`crossfade`、`timing`、`validation` セクション
...
- BGMテンプレート: `bgm` セクション **実装済み** — volume_percent/fade_in_seconds/fade_out_seconds/layer
  - C# CsvImportDialog: テンプレートからBGM音量・フェード値を自動読み込み
  - fadeIn/fadeOut 分離対応 (従来は同一値)
- テンプレートバリアント: 3種のプリセットを提供
  - `style_template.json` (default): 標準設定。font_size 48、ken_burns 1.05x、crossfade 0.5秒、BGM 30%
  - `style_template_cinematic.json`: 大きめフォント(56)、暖色系、ゆっくりアニメーション(1.08x)、長めフェード(3秒)、max_total 7200秒
  - `style_template_minimal.json`: 小さめフォント(40)、モノクロ系(4色)、高速テンポ(crossfade 0.3秒)、bold無効
  - CLI: `--style-template <path>` で任意のJSONを指定可能
  - 残: 実運用での cinematic/minimal 動画生成検証
...
| 2026-03-17 | **Phase 4完了**: SP-031 BGMテンプレート自動配置 (bgmセクション+Python/C#双方+fadeIn/fadeOut分離) |
