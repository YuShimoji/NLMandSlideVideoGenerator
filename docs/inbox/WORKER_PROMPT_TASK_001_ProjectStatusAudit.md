# Worker Prompt: TASK_001_ProjectStatusAudit
...
- 完了済み: Phase 0-6（初期セットアップ完了）
- 新規タスク: TASK_001_ProjectStatusAudit（Status: OPEN, Tier: 1）
</mission_log>
...
Phase 2: 境界
- Focus Area: 
  - プロジェクトの現在の状態確認（テストスイートの実行結果確認、主要機能の動作確認、依存関係の整合性確認）
  - 環境診断（sw-doctor.js の実行と結果分析、既存のドキュメント構造の確認、バックログと実装状況の整合性確認）
  - 次のタスクの優先順位決定（docs/backlog.md の確認、未完了タスクの整理、優先度の高いタスクの特定）
- Forbidden Area: 
  - 既存コードの大幅な変更
  - 新機能の実装
  - テストスイートの変更（確認のみ）
...
4. チケット: docs/tasks/TASK_001_ProjectStatusAudit.md（**存在確認: `Test-Path docs/tasks/TASK_001_ProjectStatusAudit.md` または `ls docs/tasks/TASK_001_ProjectStatusAudit.md`**）
...
1. Tier: 1
2. Branch: main
3. Report Target: docs/inbox/REPORT_TASK_001_ProjectStatusAudit_2026-01-05T01-14-04Z.md
...
<step>
1. 完了時: `Done: docs/tasks/TASK_001_ProjectStatusAudit.md. Report: docs/inbox/REPORT_TASK_001_ProjectStatusAudit_2026-01-05T01-14-04Z.md. Tests: <cmd>=<result>.`
2. ブロッカー継続時: `Blocked: docs/tasks/TASK_001_ProjectStatusAudit.md. Reason: <要点>. Next: <候補>. Report: docs/inbox/REPORT_TASK_001_ProjectStatusAudit_2026-01-05T01-14-04Z.md.`
...
<stop_output>
停止時の必須アウトプット:
1. チケット docs/tasks/TASK_001_ProjectStatusAudit.md を IN_PROGRESS/BLOCKED のまま更新  
...
4. チャット 1 行: `Blocked: docs/tasks/TASK_001_ProjectStatusAudit.md. Reason: <要点>. Next: <候補>. Report: docs/inbox/REPORT_TASK_001_ProjectStatusAudit_2026-01-05T01-14-04Z.md.`
5. MISSION_LOG.md を更新（停止理由と次手を記録）。
</stop_output>

<output_format>
納品レポート（docs/inbox/REPORT_...md）フォーマット:
# Report: プロジェクト状態確認と環境診断

**Timestamp**: <ISO8601>  
**Actor**: Worker  
**Ticket**: docs/tasks/TASK_001_ProjectStatusAudit.md  
**Type**: Worker  
**Duration**: <所要時間>  
**Changes**: <変更量要約>
