...
| ファイル | 内容 |
|---------|------|
| `NLMSlidePlugin.csproj` | .NET 10 プラグインプロジェクト定義（YMM4 サンプル準拠） |
...
| ドキュメント | 変更内容 |
|-------------|---------|
| `docs/ymm4_poc_plan.md` | プラグイン API 優先アプローチに更新、シナリオ Zero 追加。後続で .NET 10 SDK 前提と README へのテスト手順集約を追記 |
| `docs/ymm4_integration_arch.md` | プラグイン優先・AutoHotkey 代替の方針を反映 |

### 1.4 本セッションで実施したビルド・テスト

- `winget` による **.NET 10 SDK (10.0.100)** のインストール
- `ymm4-plugin/NLMSlidePlugin.csproj` を YMM4 公式 `YMM4SamplePlugin.csproj` 準拠の `net10.0-windows10.0.19041.0` ターゲットに更新
- `ymm4-plugin` ディレクトリで `dotnet build` を実行し、ビルド成功を確認
  - 出力: `$(YMM4DirPath)user\plugin\NLMSlidePlugin\NLMSlidePlugin.dll`
- YMM4 側でのプラグイン一覧確認は次回タスク（Y4-1 シナリオ Zero の後半）で実施予定
...
1. **環境準備**:
   - .NET 10 SDK インストール（YMM4 プラグインサンプルに追随）
...
1. **.NET 10 SDK 必須**（現在の `NLMSlidePlugin.csproj` 設定に合わせる）
2. **YMM4 アセンブリ参照**:
   - `YukkuriMovieMaker.Plugin.dll`（必要に応じて `YukkuriMovieMaker.Controls.dll` などを追加）
3. **ビルド後自動コピー**: `$(YMM4DirPath)user\plugin\NLMSlidePlugin\` へ
