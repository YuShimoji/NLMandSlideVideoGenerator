...
> **注**: この仕様は削除済みの PIL TextSlideGenerator (`src/core/visual/text_slide_generator.py`) を対象としたものです。
> TextSlideGenerator は Google Slides API 移行決定 (DECISION 2026-03-22) に伴い削除されました。
> 本文書は歴史的参照として保持しています。
...
- TextSlideGenerator (text_slide_generator.py): PIL/Pillow ベースの画像生成 — **削除済み**
...
SegmentClassifier の分類結果 (visual/textual) を TextSlideGenerator (削除済み) に渡し、
レイアウト選択に活用する。
...
- `src/core/visual/text_slide_generator.py` — **削除済み**
- `config/settings.py` — PLACEHOLDER_THEMES 拡張
- `tests/test_text_slide_generator.py` — **削除済み**
