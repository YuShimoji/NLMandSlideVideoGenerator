...
最終更新: 2025-12-08
対象バージョン: API なし CSV タイムラインモード / YMM4 エクスポート基盤完了後
...
- 今後実装するスクリプト／AutoHotkey／REST API などの **接続ポイント** を整理する
- 既存ドキュメント（`backlog.md`, `ymm4_export_spec.md`, `user_guide_manual_workflow.md` 等）との整合を取る

本ドキュメントはあくまで **API なし運用を前提とした YMM4 連携アーキテクチャ** を対象とし、
将来の YMM4 REST API 仕様が固まった際には、別途「API 連携詳細設計」として拡張する想定です。
...
| YMM4 操作自動化 (AutoHotkey) | YMM4 起動、台本 CSV インポート、音声生成、書き出し操作を自動化/半自動化 | C2/C3-4 系タスク |
...
- この段階では、
  - 本アプリと YMM4 を **疎結合** に保つため、
    - 本アプリから PoC スクリプトを直接呼び出さなくてもよい（手動実行を前提とする）
    - 代わりに「手順書 + スクリプト実体」を提供する形でスタートする。
