...
- **プラグイン API 詳細仕様の公開範囲**
  - 2025-12-11 時点では `IVoicePlugin` / `ITextCompletionPlugin` のメソッドシグネチャが公開ドキュメントからは完全には読み取れない。
  - そのため、backlog の C3-1/C3-2（プラグインAPIクライアント実装/タイムライン挿入）は「仕様不足のため一旦保留」とし、NotebookLM/Gemini/TTS 周辺の API フェーズを優先する。
  - 将来的には YMM4 アセンブリのリフレクションや追加サンプル公開などで仕様を補完した上で再開する。
- **自動化レベルの段階的向上**
