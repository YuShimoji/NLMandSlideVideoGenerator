...
[Step 7]  確定台本 → CSV 出力
    ↓
[既存] CSV → YMM4 (NLMSlidePlugin CSVインポート) → mp4
```
...
> **優先順** (DESIGN_FOUNDATIONS.md Section 0):
> 1が正規経路。2, 3はフォールバック。

1. **テキストファイル** (`.txt`) — **正規経路**: NotebookLM Audio Overview → テキスト化の出力
2. **CSV** — 既にCSV形式に整形済みの台本
3. **JSON** — Gemini生成のスクリプトJSON (NLMテキスト未提供時のフォールバック)
...
- [x] Gemini を使った要約比較 (`_async_llm_alignment` — orphaned→supported rescue)
- [x] AlignmentReport の生成ロジック

### Phase 3: レビューUI（2-3日）
- [x] CLI での採否判断フロー (`research_cli.py review` — 対話/autoモード)
- [x] Web UI での差分表示 + 編集
- [x] 確定CSV のエクスポート

### Phase 4: 統合テスト
- [x] サンプルトピックでの End-to-End テスト (`tests/test_research_e2e.py`)
- [x] collect → align → review → CSV の一気通貫テスト
- [x] 既存 CSV パイプラインとの結合確認 (CsvAssembler統合済み)

### Phase 5: CLI pipeline サブコマンド
- [x] `research_cli.py pipeline` — collect → script gen (Gemini) → align → review → CsvAssembler の一気通貫実行
- [x] `--auto-review` オプション (supported→adopted, それ以外→rejected)
- [x] `--slides-dir` / `--speaker-map` によるCsvAssembler連携
- [x] テスト: `tests/test_research_pipeline.py` (3件)
