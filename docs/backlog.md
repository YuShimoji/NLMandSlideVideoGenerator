...
| 優先度 | ID | 内容 | 工数 | 状態 | 備考 |
|--------|-----|------|------|------|------|
| | **E2E検証** | フォールバック戦略でE2E動画書き出し検証 | | | |
| | **D-10** | FFmpeg検出・インストール案内改善 | | | `ffmpeg_utils.py` |
| | **C-1-2** | CSV Web フォーム作成 | | | Web UIに追加 |
| | **U-1** | ユーザー向けワークフローガイド | | | `user_guide_manual_workflow.md` |
| | **U-2** | CSV入力フォーマット仕様書 | | | `spec_csv_input_format.md` |
| | **A-3** | Google Slides API実装 | | API設定待ち | 準備完了、認証待ち |
| | **A-1** | NotebookLM代替実装 | | API設定待ち | Gemini APIキー必要 |
| | **C3-5** | テンプレート差分適用整備 | | 待機 | YMM4カスタマイズ |
...
| 項目 | 結果 | 備考 |
|------|------|------|
| ExportFallbackManager | | バックエンド検出・切替動作 |
| 字幕生成 | | SRT/ASS/VTT出力 |
| フォールバック動作 | | MoviePy→FFmpeg |
| 動画出力 | | FFmpeg未検出時は空ファイル |
...
| 優先度 | ID | 内容 | 工数 | 状態 |
|--------|-----|------|------|------|
| | **S-1** | サンプルCSV/WAVセット作成 | 30分 | |
| | **S-2** | READMEクイックスタート追記 | 30分 | |
| | **S-3** | エラーメッセージ改善（CSV Web UI） | 1時間 | |
| | **S-4** | Web UI 進捗表示の改善 | 1-2時間 | |
| | **S-5** | 環境チェックのWeb UI統合 | 1時間 | |

### 推奨される次のタスク（APIなし・短期・UX改善）

| 優先度 | ID | 内容 | 工数 | 効果 | 状態 |
|--------|-----|------|------|------|------|
| | **S-6** | 音声ファイルのWeb UIドラッグ＆ドロップ対応 | 1-2時間 | 素材投入UX大幅改善 | |
| | **S-7** | Web UI結果画面強化（動画プレビュー・リンク） | 1時間 | 結果確認UX改善 | |
| | **S-8** | 入力素材プレビュー（CSV/WAV一覧表示） | 1時間 | ミスマッチ事前警告 | |
| | **S-9** | YouTube用サムネイル自動生成 | 2時間 | 投稿準備の省力化 | |
| | **S-10** | YouTube用メタデータ自動生成（APIなし） | 1時間 | 投稿準備の省力化 | |

### 推奨される次のタスク（APIなし・中期・ビジュアル改善）

| 優先度 | ID | 内容 | 工数 | 効果 | 状態 |
|--------|-----|------|------|------|------|
| | **M-1** | CSVパイプラインの統合テスト追加 | 2時間 | | |
| | **M-2** | YMM4テンプレート差分適用 (C3-5) | 2-3時間 | | |
| | **M-3** | 音声ファイル自動分割ツール | 2-3時間 | 素材準備支援 | |
| | **M-4** | プレースホルダースライドのテーマ化 | 2時間 | ビジュアル改善 | |
| | **M-5** | 字幕ハードサブ環境ガイド＆オプション | 1時間 | pysrt有無の明確化 | |
