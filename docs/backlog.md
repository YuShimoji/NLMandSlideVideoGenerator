...
| フェーズ | 目的 | 状態 |
|----------|------|------|
| A | NotebookLM / Slides 実装・整備（コア機能） | � API連携フェーズ待ち（A-4完了、Gemini APIキー設定済） |
| B | Web / API 運用性向上（UI・ジョブ管理） | ✅ 完了（B-1, B-2） |
| C | 新モード UX 向上（CSV / YMM4） | � 基盤完了、YMM4プラグインシナリB待ち |
...
| TASK_022 | VOICEVOX統合 — CLOSED (WONTFIX): YMM4一本化により不要。コード全削除済 | 2026-03-04 |
| TASK_024 | pipeline.py リファクタリング (1384→431行, -69%) | 2026-03-04 |
