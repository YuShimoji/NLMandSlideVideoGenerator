"""
Research UI の最小E2E確認を Playwright で行う。
"""
...
def _wait_for_export_ready(page, timeout: float = 60.0):
    ...
    while time.time() < deadline:
        ...
        if error_text.count() > 0 and error_text.first.is_visible():
            raise RuntimeError("Research UI reported an application error")
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                inputs.nth(0).set_input_files(str(SAMPLE_PACKAGE))
                inputs.nth(1).set_input_files(str(SAMPLE_SCRIPT))
        ...
        if not EXPECTED_CSV.exists():
            raise FileNotFoundError(EXPECTED_CSV)
        print(f"SMOKE_OK {EXPECTED_CSV}")
        return 0
    finally:
        process.terminate()
        try:
